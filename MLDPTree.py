...
class MLDPTree:
    ...
    def expand(self):
        ...
        if len(self.remaining) == 0 or len(set(self.data[self.class_attr])) <= 1:
            ...
            for (a, v) in split_values:
                ...
                # Construct subtree
                n = MLDPTree(a, v, (MLDPTree(data=d0, remaining=rc, classes=self.classes, class_attr=self.class_attr),
                                    MLDPTree(data=d1, remaining=rc, classes=self.classes, class_attr=self.class_attr)),
    ...
    def train(data, class_attr='Class'):
        return MLDPTree(data=data, class_attr=class_attr).expand().prune()
